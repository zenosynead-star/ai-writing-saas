...
6. **本文は合計 ${vars.targetChars ? vars.targetChars : 3500} 文字以上（必須・"目安"ではない）**、ただし **冗長な水増しは厳禁で ${(vars.targetChars || 3500) + 2000} 文字程度を上限の目安**に密度高くまとめる（長すぎる記事は読了率が下がる）。競合を内容量・網羅性で上回ることを最優先に、具体例・手順・数値・比較・FAQ で実質的に充実させる
...
/** 1つの h2 セクション(h2とその配下)だけを按分目標まで増補するプロンプト（大目標のセクション単位増補用）。 */
export const EXPAND_SECTION_PROMPT = (vars: {
  title: string;
  sectionHtml: string;
  targetChars: number;
}) => `# 役割
あなたはSEO記事の編集者です。記事「${vars.title}」の以下の「1セクション」(先頭の<h2>とその配下)を、競合を上回る具体性で増補してください。

# 増補ルール（厳守）
1. **先頭の <h2> と、既存の <h3>/本文/内部リンク(<a>)/装飾(<strong>/<mark>/<blockquote>/<table>/<ul>/<ol>)はそのまま残す**（削除・改変しない）
2. 具体例・手順・数値・比較表・箇条書き・FAQ・注意点・体験的視点を加えて自然に厚くする（水増し・同義反復は禁止）
3. 文体は です・ます調。事実は正確に、不確かな数値は書かない
4. 重要語は <strong>、最重要文は <mark> でハイライト。比較・手順・料金は <table>/<ul> で構造化
5. **このセクション単体で ${vars.targetChars} 文字以上**になるまで充実させる
6. 新しい <h2> は作らない・他セクションの話題に踏み込まない（この h2 セクションの中身だけを深掘りする）

# 入力（このセクションHTMLを増補して返す）
${vars.sectionHtml}

# 出力形式
増補後の**このセクションのHTMLのみ**を出力する。必ず先頭の <h2> から始め、前後に説明文・マークダウン・コードフェンスを付けない。`;
